						...
						/*
							<PACKAGE_ID>: {
											<YEAR>:
											        {
														<MONTH>: {
																	status:  <numeric>,   // 0 = missing data, 1 = stale, 2 = okay
							  	...
							  	if(data && data.rows) {
								  	...
								  	_.forEach(data.rows, function(row) {
								  		if(! row.id.startsWith('_')) {
								  			// Sample row:
								  			// {	
								  			//	"id":"nosqlimport-mongodb_2016-07",
								  			//  "key":"nosqlimport-mongodb_2016-07",
								  			//  "value":{"rev":"1-ed7553270bed2234bfbe94e3025b96ed"}
								  		    // }
								  			// 
												...
												if(! this.statsIndex[packageyearmonth[1]][packageyearmonth[2]].hasOwnProperty(packageyearmonth[3])) {
													if(currentdate === (packageyearmonth[2] + '-' + [packageyearmonth[3]])) {
									  					this.statsIndex[packageyearmonth[1]][packageyearmonth[2]][packageyearmonth[3]] = 
									  						{
									  							status: 1,            // data is potentially stale
									  							docid: row.id,        // document id
									  							docrev: row.value.rev // document revision
									  						};
									  				}
									  				else {
										  				this.statsIndex[packageyearmonth[1]][packageyearmonth[2]][packageyearmonth[3]] = 
										  					{
									  							status: 2,            // data is okay
									  							docid: row.id,        // document id
									  							docrev: row.value.rev // document revision
									  						};
									  				}
									...
									_.forEach(months,
											  function(month) {
												...
												if(! this.statsIndex[packageid][year].hasOwnProperty(month)) {
													...
													if(this.statsIndex[packageid][year][month].status === 1) {

...
var configureMetaRepository = function(metaRepository, options, callback) {
    ...
    async.series(
    				[
    					...
    					function(asyncCallback) {
    						const configDoc = config.getDefaultConfig(options);
	...
	async.parallel({
						data: function(asyncCallback) {
							const designDoc = {
											    ...
											    views: {
											     ...
											     packages: {
											       map: 'function(doc) {if (doc.type === \'stats\') {var stale = false;var cutoffdate = new Date(doc.month.substring(0,4), doc.month.substring(5,7), 2).toISOString();if((doc.month === doc.collection_date.substring(0,7)) || (doc.collection_date <= cutoffdate )) {stale = true;} emit([doc.package, doc.month, doc._rev], stale); }}'
											     },
											     list: {
											       map: 'function(doc) {if(doc.type === \'stats\') { emit(doc.package, doc.total); }}',		
											       reduce: '_stats'	
											     }    
											   },
											   language: 'javascript'
				   ...
				   function(err, handles) {
						...
						if(err) {
							...
							return initCallback(null, handles.data, handles.meta, new Collector(handles.data), new Indexer(handles.data), new config.Config(handles.meta));
						}
					});
};
